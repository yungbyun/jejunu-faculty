...
**퀴즈 — 이름 입력칸 삭제, 앞뒤 이동 (2026-09-20).** 사진 밑 입력칸(`#qzIn`)을 없앴습니다.
답은 마이크로 말한 뒤 `확인` 으로 제출하며, `확인` 은 들은 말이 있을 때만 눌립니다.
**음성 인식이 안 되는 브라우저에서는 답을 제출할 길이 없으므로** 폼 대신 안내문을 띄우고
힌트·정답 보기로 진행하게 했습니다. `← 이전` 버튼과 사진을 오른쪽으로 미는 동작으로 앞 문제로
돌아갑니다. 문제별 상태(`hints/wrong/state/gained/heard`)는 `quiz.res[i]` 에 넣었다 꺼내므로
되돌아가도 점수가 두 번 오르지 않고, 건너뛴 문제는 다시 풀 수 있습니다.
관련 함수: `quizSnap / quizGo / quizNext / quizPrev / quizStateAt`.

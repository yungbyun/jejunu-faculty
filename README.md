...
학과 화면의 교수 카드 오른쪽 위(모바일은 카드 하단)와 상세 창에서 `상 / 중 / 하 / 부`를 고를 수 있습니다. **부**는 부정(negative)으로 가장 낮은 선호도입니다. 같은 값을 다시 누르면 해제됩니다. 선호도는 로그인한 계정별로 따로 저장되며, 학과 화면의 선호도 필터와 첫 화면의 학과별 집계에 반영됩니다.
...
상단 메뉴의 **분석**(`#/stats`)은 로그인한 계정의 선호도를 시각화합니다. 평가 진행률과 상·중·하·부·미지정 집계, 학과별 100% 누적막대(구간을 누르면 해당 학과가 그 선호도 필터로 열림), 전공 키워드별 평균 점수(상 3 · 중 2 · 하 1 · 부 −1, 미지정 제외)의 다이버징 막대, 직급별·학과별 표, 선호도별 교수 목록과 CSV 내보내기로 구성됩니다. 외부 차트 라이브러리 없이 HTML/CSS로 그리며, 시트 연동 여부와 관계없이 브라우저에 있는 선호도만으로 동작합니다.
